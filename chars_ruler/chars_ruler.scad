...
module characters() {
  ...
  color("blue") translate(
      [ char_start_x_offset, y_depth - chars_y_top_offset - char_font_size, 0 ])
      linear_extrude(z_height) for (row = [0:len(chars)]) {
    translate([ 0, -row * char_font_size * 1.5, 0 ])
        text(chars[row], char_font_size,
...
module chanfer() {
  translate([ x_width, 80, 2.5 ]) rotate([ 180, 90, 0 ]) linear_extrude(x_width)
...
module ruler() {
  difference() {
    ...
    color("purple") translate([ 180, 10, z_height - scale_line_depression ])
        linear_extrude(scale_line_depression)
            text("OLÍVIA", 8, font = "BigShouldersStencilText:style=Medium");

    color("pink") translate([ 157, 10 ]) linear_extrude(z_height)
        text("✸♥", 8, font = "Noto");
  }
}
